...
function getTable(config) {
  ...
  if (counter === 0) {
    ...
    options = utils.post_options(
      `PassSummary.aspx?satid=${config.target}&`,
      opt
    );
  }
  request(options, (error, response, body) => {
    ...
    function factory(temp) {
      return new Promise((resolve, reject) => {
        request(
          utils.image_options(temp[property[0]]),
          (error, response, body) => {
            ...
            for (let i = 0; i < tbody.find("tr").length; i++) {
              if (
                tbody.find("tr").eq(i).find("td").eq(0).text() === "离开地影"
              ) {
                //Exits shadow
                ...
                tbody.find("tr").eq(i).find("td").eq(0).text() === "进入地影"
              ) {
                //Enters shadow
            ...
            temp[property[5]] =
              "https://www.heavens-above.com/" +
              $("#ctl00_cph1_imgViewFinder").attr("src"); //.replace("size=800", "size=1600");
            ...
            request
              .get(utils.image_options(temp[property[5]]))
              .pipe(
